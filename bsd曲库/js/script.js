...
function initSearch() {
    ...
    const searchData = {
        artists: [
            ...
            { id: 4, name: '万能青年旅店', image: '${image_url_placeholder}', type: 'artist' }
        ],
        albums: [
...
function navigateToArtistPage(card) {
    ...
    document.body.style.animation = 'fadeIn 0.3s ease reverse';
    setTimeout(() => {
        if (artistName === '万能青年旅店') {
            window.location.href = 'artist4.html';
        } else {
...
// 导航到单曲页面
function navigateToSinglePage(singleName) {
    // 页面切换动画
    document.body.style.animation = 'fadeIn 0.3s ease reverse';
    setTimeout(() => {
        // 这里可以根据单曲名称跳转到不同的单曲详情页面
        // 目前暂时跳转到一个通用的单曲详情页面
        window.location.href = `single.html?name=${encodeURIComponent(singleName)}`;
    }, 300);
}
